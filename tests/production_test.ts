...
Deno.test(
  "Retry - withRetry retries on lock timeout",
  testOptions,
  async () => {
    let attempts = 0;
...
Deno.test(
  "Retry - retryable wrapper works correctly",
  testOptions,
  async () => {
    let calls = 0;
...
Deno.test(
  "Observability - MetricsAggregator collects stats",
  testOptions,
  async () => {
...
Deno.test(
  "Integrity - verifyIntegrity detects duplicate IDs",
  testOptions,
  async () => {
...
Deno.test(
  "Integrity - repairEvents removes duplicates",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Integrity - formatIntegrityReport produces readable output",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Backup - export and import roundtrip works",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
