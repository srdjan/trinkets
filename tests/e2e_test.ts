...
Deno.test("E2E - Complete feature development lifecycle", testOptions, async () => {
...
Deno.test("E2E - Complex dependency graph management", testOptions, async () => {
  const tempDir = await Deno.makeTempDir();
  try {
    const store = await openJsonlStore({ baseDir: tempDir });
...
Deno.test("E2E - Simulated real project with 50 issues", testOptions, async () => {
  const tempDir = await Deno.makeTempDir();
  try {
