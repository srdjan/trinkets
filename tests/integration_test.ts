...
import { addLink, createIssue, patchIssue, setStatus } from "../src/domain.ts";
...
Deno.test(
  "Integration - JSONL store persists events",
  testOptions,
  async () => {
...
Deno.test(
  "Integration - JSONL store appends events sequentially",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Integration - JSONL store survives reopening",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Integration - HeadsV2 store uses incremental replay",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Integration - HeadsV2 store survives reopening with cache",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test("Integration - HeadsV2 with SQLite cache", testOptions, async () => {
  ...
  try {
    ...
    const i1 = await createIssue(store, testEnv, {
      title: "Task 1",
      priority: 1,
    });
    const i2 = await createIssue(store, testEnv, {
      title: "Task 2",
      priority: 2,
    });
    const i3 = await createIssue(store, testEnv, {
      title: "Task 3",
      priority: 3,
    });
...
Deno.test(
  "Integration - HeadsV2 handles 100 events with caching",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
...
Deno.test(
  "Integration - getExistingIds optimizes ID collision checks",
  testOptions,
  async () => {
    const tempDir = await Deno.makeTempDir();
    try {
